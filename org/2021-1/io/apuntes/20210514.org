...
- [[#técnica][Técnica]]
  ...
  - [[#cpm-método-ruta-critica][CPM: Método Ruta Critica]]
    ...
    - [[#procedimiento][Procedimiento]]
      - [[#método][Método]]
        - [[#next-wait-for-this-lecture-to-be-uploaded][NEXT Wait for this lecture to be uploaded]]
  - [[#pert][PERT]]
...
|-----+--------+-----|
- H: Holgura
  + Si H=0 -> Actividad Critica, \(H \geq 0\)


- Actividad Inicio y Fin tienen duración 0
...
#+begin_center
Se rellenan en orden:
1) D: vienen dado en los enunciados
   - \(ES_{Inicio}=D=0\)
2) ES
   - Rellenar en actividades iniciales
3) EF=ES+D
   - Rellenar en actividades iniciales
   - Luego la siguientes actividades, si tienen dependencias:
   - \(ES_{actividad a rellenar} = max(EF_1,...,EF_n) de las dependencias\)
4) Se llega al final: donde en la actividad Fin:
   1) \(ES_{Última actividad}=EF_{Fin}\)
      2)


Luego, se empieza a rellenar de adelante hacia atras.
...
***** NEXT Wait for this lecture to be uploaded
- [2021-06-07 Mon]
** PERT
